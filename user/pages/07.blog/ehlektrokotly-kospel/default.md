---
title: 'Модели котлов EKCO'
---

<div class="container-fluid mt-5 mb-5">
<section class="bg-primary text-white fw-bold mb-5">
<div class="py-5">
<div class="container text-center">

<p class="h4 lead text-white">Надежные и эффективные отопительные котлы для вашего дома</p>
</div>
</div>
</section>
<div class="card mb-5">
<div class="card-body">
<p class="fs-5">Котлы EKCO предназначены для обогрева помещений, они могут быть самостоятельными источниками тепла или совместно работать с другими котлами.</p>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-info text-white">
<p class="card-h3 text-white fs-5"><span class="icon me-2"><i class="fas fa-list"></i></span>Модели котлов EKCO</p>
</header>
<div class="card-body p-0">
<div class="table-container">
<table class="table is-bordered is-striped w-100 text-center mb-0">
<thead class="bg-dark text-white">
<tr>
<th>Модель</th>
<th>Изображение</th>
<th>Описание</th>
</tr>
</thead>
<tbody>
<tr>
<td>**Котел EKCO.T**</td>
<td><a href="kotel-kospel1.jpg" target="_blank" data-h3="Котел EKCO.T" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel1.jpg" alt="Котел EKCO.T" class="image is-128x128 is-inline-block" style="max-height: 150px;"> </a></td>
<td>Котел большой мощности для совместной работы в отопительной системе, а также с бойлером ГВС</td>
</tr>
<tr>
<td>**Котел EKCO.MNz**</td>
<td><a href="kotel-kospel2.jpg" target="_blank" data-h3="Котел EKCO.MNz" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel2.jpg" alt="Котел EKCO.MNz" class="image is-128x128 is-inline-block" style="max-height: 150px;"> </a></td>
<td>Котел с наружным погодным датчиком, большой мощности, для совместной работы с одним или двумя отопительными контурами и бойлером ГВС</td>
</tr>
<tr>
<td>**Котел EKCO.L2**</td>
<td><a href="kotel-kospel3.jpg" target="_blank" data-h3="Котел EKCO.L2" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel3.jpg" alt="Котел EKCO.L2" class="image is-128x128 is-inline-block" style="max-height: 150px;"> </a></td>
<td>Котел для системы типа «теплый пол»; cниженные параметры обогрева и соответствующая термическая защита</td>
</tr>
</tbody>
</table>
</div>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-success text-white">
<p class="card-h3 text-white fs-5"><span class="icon me-2"><i class="fas fa-fire"></i></span>Котлы EKCO.T / EKCO.TM</p>
</header>
<div class="card-body">
<div class="row g-3">
<div class="col-4 mb-3 text-center"><a href="https://boilererror.ru/wp-content/uploads/2019/12/kotel-kospel1.jpg" target="_blank" data-h3="Котлы EKCO.T / EKCO.TM" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel1-a6613be3.jpg" alt="Котлы EKCO.T / EKCO.TM" class="is-fullwidth has-border-light"> </a></div>
<div class="col is-two-thirds content">
<ul class="ml-4 list-unstyled">
<li class="mb-2"><span class="icon me-2 text-primary"><i class="fas fa-bolt"></i></span> **Модель EKCO.T** — котел большой мощности для совместной работы в отопительной системе, а также с бойлером ГВС</li>
<li class="mb-2"><span class="icon me-2 text-success"><i class="fas fa-thermometer-half"></i></span> **Модель EKCO.TM** – котел с наружным погодным датчиком, для работы с одним или двумя отопительными контурами и бойлером ГВС</li>
<li class="mb-2"><span class="icon me-2 text-warning"><i class="fas fa-layer-group"></i></span> **Возможность работы в каскаде** - EKCO.TM (ведущий) и EKCO.T (ведомые)</li>
<li class="mb-2"><span class="icon me-2 text-info"><i class="fas fa-sliders-h"></i></span> **Регулирование температуры воды** в отопительной системе в пределах 40 — 85°C</li>
<li class="mb-2"><span class="icon me-2 text-danger"><i class="fas fa-cogs"></i></span> **Оборудованы 2 нагревательными узлами**, что снижает нагрузку и обеспечивает длительное использование</li>
</ul>
</div>
</div>
<div class="text-center mt-4"><a href="kotel-kospel9.jpg" target="_blank" data-h3="Дополнительное изображение EKCO.T" class="is-inline-block" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel9.jpg" alt="Дополнительное изображение EKCO.T" style="max-height: 200px;" class="has-border-light"> </a></div>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-warning text-dark">
<p class="card-h3 text-dark fs-5"><span class="icon me-2"><i class="fas fa-temperature-high"></i></span>Котлы EKCO.R2</p>
</header>
<div class="card-body">
<div class="row g-3 is-vcentered">
<div class="col-4 mb-3 text-center"><a href="kotel-kospel8-d8eb466a.jpg" target="_blank" data-h3="Котлы EKCO.R2" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel8-d8eb466a.jpg" alt="Котлы EKCO.R2" class="is-fullwidth has-border-light"> </a></div>
<div class="col is-two-thirds content">
<ul class="ml-4 list-unstyled">
<li class="mb-2"><span class="icon me-2 text-primary"><i class="fas fa-home"></i></span> **Предназначены** для совместной работы в отопительных системах, а также с бойлером косвенного нагрева</li>
<li class="mb-2"><span class="icon me-2 text-success"><i class="fas fa-gamepad"></i></span> **Панель управления** – установка желаемого значения температуры теплоносителя в диапазоне **30 — 85 °C**</li>
<li class="mb-2"><span class="icon me-2 text-warning"><i class="fas fa-fire"></i></span> **Нагревательный узел** из нержавеющей стали (кроме модели с 24 кВт — из меди)</li>
<li class="mb-2"><span class="icon me-2 text-info"><i class="fas fa-clock"></i></span> **Комнатный программатор** контролирует температуру в помещении и высылает сигнал включения/выключения</li>
</ul>
</div>
</div>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-danger text-white">
<p class="card-h3 text-white fs-5"><span class="icon me-2"><i class="fas fa-water"></i></span>Котлы EKCO.L2 / EKCO.LN2 (Для теплых полов)</p>
</header>
<div class="card-body">
<div class="row g-3 is-vcentered">
<div class="col-4 mb-3 text-center"><a href="kotel-kospel4-a8dfa6c9.jpg" target="_blank" data-h3="Котлы EKCO.L2 / EKCO.LN2" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel4-a8dfa6c9.jpg" alt="Котлы EKCO.L2 / EKCO.LN2" class="is-fullwidth has-border-light"> </a></div>
<div class="col is-two-thirds content">
<ul class="ml-4 list-unstyled">
<li class="mb-2"><span class="icon me-2 text-primary"><i class="fas fa-tint"></i></span> **Модель EKCO.LN2** – котел оборудован расширительным баком (6 литров) и обходным клапаном bypass</li>
<li class="mb-2"><span class="icon me-2 text-success"><i class="fas fa-tint-slash"></i></span> **Модель EKCO.L2** – идентичен EKCO.LN2, но **без** расширительного бака и клапана bypass</li>
<li class="mb-2"><span class="icon me-2 text-warning"><i class="fas fa-shoe-prints"></i></span> **Модель EKCO.LN2p** — котел для системы типа **«теплый пол»** (сниженные параметры обогрева, есть бак и bypass)</li>
<li class="mb-2"><span class="icon me-2 text-info"><i class="fas fa-shoe-prints"></i></span> **Модель EKCO.L2p** – идентичен EKCO.LN2р, но **без** расширительного бака и клапана bypass</li>
</ul>
</div>
</div>
<div class="text-center mt-4"><a href="kotel-kospel6.jpg" target="_blank" data-h3="Дополнительное изображение EKCO.L2" class="is-inline-block me-3" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel6.jpg" alt="Дополнительное изображение EKCO.L2" style="max-height: 150px;" class="has-border-light"> </a> <a href="kotel-kospel5.jpg" target="_blank" data-h3="Котлы EKCO.L2 / EKCO.LN2" class="is-inline-block" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel5.jpg" alt="Котлы EKCO.L2 / EKCO.LN2" style="max-height: 150px;" class="has-border-light"> </a></div>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-secondary text-white">
<p class="card-h3 text-white fs-5"><span class="icon me-2"><i class="fas fa-cloud-sun"></i></span>Котлы EKCO.Мz / EKCO.МNz (Погодозависимые)</p>
</header>
<div class="card-body">
<div class="row g-3 is-vcentered">
<div class="col-4 mb-3 text-center"><a href="https://boilererror.ru/wp-content/uploads/2019/12/kotel-kospel2.jpg" target="_blank" data-h3="Котлы EKCO.Мz / EKCO.МNz" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel2-1e3110a4.jpg" alt="Котлы EKCO.Мz / EKCO.МNz" class="is-fullwidth has-border-light"> </a></div>
<div class="col is-two-thirds content">
<ul class="ml-4 list-unstyled">
<li class="mb-2"><span class="icon me-2 text-primary"><i class="fas fa-thermometer-empty"></i></span> **Модель EKCO.Mz** — котел с наружным погодным датчиком, без расширительного бака.</li>
<li class="mb-2"><span class="icon me-2 text-success"><i class="fas fa-thermometer-full"></i></span> **Модель EKCO.MNz** — котел с наружным погодным датчиком, **оснащен расширительным баком** (6 литров).</li>
<li class="mb-2"><span class="icon me-2 text-warning"><i class="fas fa-sync-alt"></i></span> **Погодозависимое управление** — котел автоматически реагирует на изменение температуры снаружи для экономной эксплуатации.</li>
</ul>
</div>
</div>
<div class="text-center mt-4"><a href="kotel-kospel7.jpg" target="_blank" data-h3="Дополнительное изображение EKCO.Мz / EKCO.МNz" class="is-inline-block" rel="noopener"> <img src="/blog/ehlektrokotly-kospel/kotel-kospel7.jpg" alt="Дополнительное изображение EKCO.Мz / EKCO.МNz" style="max-height: 200px;" class="has-border-light"> </a></div>
</div>
</div>
<div class="card mb-5"><header class="card-header bg-light">
<p class="card-h3 fs-5" style="color: #e37a25;"><span class="icon me-2"><i class="fas fa-thumbs-up"></i></span>Преимущества котлов EKCO</p>
</header>
<div class="card-body">
<div class="row text-center">
<div class="col is-one-quarter mb-3"><span class="icon btn-lg mb-2 text-primary"><i class="fas fa-bolt fa-3x"></i></span>
<h5 class="h6">Надежность</h5>
<p class="fs-6 mb-0">Стабильная работа и долговечность</p>
</div>
<div class="col is-one-quarter mb-3"><span class="icon btn-lg mb-2 text-success"><i class="fas fa-sliders-h fa-3x"></i></span>
<h5 class="h6">Управляемость</h5>
<p class="fs-6 mb-0">Точное регулирование температуры</p>
</div>
<div class="col is-one-quarter mb-3"><span class="icon btn-lg mb-2 text-warning"><i class="fas fa-cloud-sun fa-3x"></i></span>
<h5 class="h6">Экономичность</h5>
<p class="fs-6 mb-0">Погодозависимое управление</p>
</div>
<div class="col is-one-quarter mb-3"><span class="icon btn-lg mb-2 text-info"><i class="fas fa-cogs fa-3x"></i></span>
<h5 class="h6">Совместимость</h5>
<p class="fs-6 mb-0">Работа в каскаде с другими котлами</p>
</div>
</div>
</div>
</div>
<div class="card bg-primary text-white shadow">
<div class="text-center">
<h3 class="h4 display-4 text-white"><span class="icon me-2"><i class="fas fa-shopping-cart"></i></span>Закажите котлы и запчасти EKCO</h3>
<p class="h5 lead text-white">Подберем нужную модель и обеспечим профессиональную установку</p>
<div class="row is-centered mt-4">
<div class="col-lg-4 is-half-mobile mb-2"><a href="tel:+79262211348" class="btn btn-light btn-lg w-100"> <span class="icon me-2"><i class="fas fa-phone"></i></span> <span>Позвонить</span> </a></div>
<div class="col-lg-4 is-half-mobile"><a href="https://service04.ru/contact-us/feedback" class="btn btn-warning btn-lg w-100 text-dark"> <span class="icon me-2"><i class="fas fa-envelope"></i></span> <span>Оставить заявку</span> </a></div>
</div>
</div>
</div>
</div>